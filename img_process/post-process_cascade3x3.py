...
#verilog_input_path = path + case_name + "verilog_out_cascade3x3_ieee.txt"
verilog_input_path = path + case_name +  "verilog_out_cascade3x3_int_shift.txt"

#verilog_img_recreate_path = path + case_name+ 'verilog_rec_cascade3x3_ieee.png'
verilog_img_recreate_path = path + case_name + 'verilog_rec_cascade3x3_int_shift.png'

og_img_path = path + case_name +'test_img.png'
gray_img_path = path + case_name +'gray_img.png'
py_img_filter_path = path + case_name +'py_img_cascade3x3.png'
...
padd_img, width, height = my_functions.add_padd(gray_img, col, row, doublepad) #col=width, row=height


##########
### apply custom filter
py_img_filter = custom_filters.cascade_3x3(padd_img, width, height)
...
ver_arr = np.array(ver_pixels, dtype=np.uint8).reshape((int(height-4), int(width-4)))
