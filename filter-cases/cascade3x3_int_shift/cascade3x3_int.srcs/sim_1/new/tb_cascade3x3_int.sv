...
// cascaded filters with 3x3 window - integers - shifts 
//////

module tb_cascade3x3_int;
    ...
    //double filter => lose 4 pixels per dimension
    localparam int EXPECTED_PIXELS = (IMG_WIDTH-4) * (IMG_HEIGHT-4); 
    ...
    parameter string FILE_OUT = {path, "smeagol_verilog_out_cascade3x3_int_shift.txt"};
    ...
    initial begin
        ...
        $display("-----------------------------------------------------");
        $display("Cascaded filters window 3x3 testbench - ints& shifts ");
        $display("-----------------------------------------------------");
